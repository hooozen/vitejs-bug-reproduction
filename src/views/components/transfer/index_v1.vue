    ...
    <div class="tl-transfer-main" style="margin-right: 20px">
      ...
      <div class="tl-transfer__list">
        <el-table
          ...
          :data="isShowSearchResult ? searchResult : leftTableData"
          height="100%"
          :stripe="true"
        >
          <el-table-column
...
<script lang="ts">
  /* note
    ...
    3. 对于搜索结果，若采用 2 中的处理，可能会给用户造成困惑（由于已经选中了而不在搜索结果中展示），因此采用展示所有搜索结果的处理，
       对于已选中的，则设置选中状态；搜索结果不提供“下级”操作，既能降低组件实现复杂度，也符合交互习惯（搜索的本意就是要获取精确结果）
  */
    ...
    setup() {
      ...
      const leftTableData = computed(() => {
        ...
        res = res.filter((item: any) => {
          return !selectedPrivileges.value.some(p => p.id == item.id && p._type == item._type)
        })
        return res
      })
      ...
      const search = async () => {
        ...
        searchResult.value = orgRes.data.concat(storeRes.data.records?.map((item: any) => ({ ...item, _type: 2 })))
      ...
      const setSelected = () => {
        searchResult.value.forEach((res: any) => {
          if (selectedPrivileges.value.some(p => p.id == res.id && p._type == res._type)) {
            tableEl.value.toggleRowSelection(res, true)
          } else {
            tableEl.value.toggleRowSelection(res, false)
          }
        })
      }
      ...
      const removePrivilege = (data: any) => {
        ...
        if (isShowSearchResult) nextTick(setSelected)
      }

      const selectableSetting = (row: any, index: number) => {
        return !selectedPrivileges.value.some(p => p.id == row.id && p._type == row._type)
      }

      const getSelectedIds = (): false | string => {
        const length = selectedPrivileges.value.length
        if (!length) return false
        return selectedPrivileges.value.map(item => item.id).join(',')
      }
      ...
      return {
        ...
        selectableSetting, tableEl, searchResult,
      }
    },
  })
</script>
